...
import { withBase, withQuery } from 'url-ops'
...
export function createFetch(globalOptions: CreateFetchOptions): $Fetch {
   ...
   const $fetchRaw: $Fetch['raw'] = async function $fetchRaw(
    ...
    _options = {},
      ...
      // We override the `.json()` method to parse the body more securely with `nyxjson`
      if (responseType === 'json') {
         const data = await context.response.text()
         const parseFunction = context.options.parseResponse || nyxjson
